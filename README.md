# LightRAG for local development using Ollama and Docker Compose
...
Then make sure Ollama is prepared to run as a service able to accept requests from LightRAG Server. You can follow the instructions in the [Ollama documentation](https://ollama.com/docs/quickstart) to set up Ollama.

To ease this step, run the script prepared in the `scripts` folder:

```bash
./scripts/configure_ollama_for_docker.sh
```

Start the LightRAG Server with Docker Compose:
